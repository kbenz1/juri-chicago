...
datei = "prozessdaten.xlsx"  # Dateiname anpassen
df = pd.read_csv(datei, sep=';')  # CSV-Datei einlesen, Trennzeichen ist ;

# 🔢 Berechnung der X̄- und R-Werte
X_bar = []
R = []

for i in range(len(df)):
    # Extrahiere die fünf Werte pro Stichprobe
    values = df.iloc[i, 1:].values  # Werte 1 bis 5 aus der Zeile (Spalten 1 bis 5)
    
    # Berechne den Mittelwert (X̄) der Stichprobe
    X_bar.append(np.mean(values))
    
    # Berechne die Spannweite (R) der Stichprobe
    R.append(np.max(values) - np.min(values))

# 🔢 Anzahl der Stichproben
n = len(X_bar)

# 🔎 Berechnungen für Kontrollgrenzen (A2, D3, D4 für n=5)
A2, D3, D4 = 0.577, 0, 2.114

# 🔍 Berechnung der Mittelwerte
X_double_bar = np.mean(X_bar)  # Gesamtmittelwert von X̄
...
for i in range(n):
    ...
    # 1️⃣ Regel: Punkt außerhalb der UCL oder LCL (X̄)
    rule_1_X = "Ja" if X_bar[i] > UCL_X or X_bar[i] < LCL_X else "Nein"

    # 2️⃣ Regel: 9 aufeinanderfolgende Punkte auf einer Seite des Mittelwerts (X̄)
    ...
    rule_4_X = "Ja" if i >= 13 and all((X_bar[j] > X_bar[j-1] and X_bar[j] < X_bar[j+1]) or (X_bar[j] < X_bar[j-1] and X_bar[j] > X_bar[j+1]) for j in range(i-13, i)) else "Nein"
    ...
    rule_4_R = "Ja" if i >= 13 and all((R[j] > R[j-1] and R[j] < R[j+1]) or (R[j] < R[j-1] and R[j] > R[j+1]) for j in range(i-13, i)) else "Nein"
...
df_auswertung.to_excel("prozessauswertung.xlsx", index=False)
...
print("Die Auswertung wurde erfolgreich in 'prozessauswertung.xlsx' gespeichert!")
